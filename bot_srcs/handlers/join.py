...
def start_conv(update, context):
	...
	context_busy[0] = True
	update.message.reply_text(
		'Чтобы стать участником мероприятия, тебе необходим идентификатор. '
		'Попроси его у организатора и отправь мне, '
		'я сделаю всю остальную работу.'
...
def get_id(update, context):
	...
	if add_participant(meeting_id, user_id) == -1:
		...
		update.message.reply_text(
			'Если ты хочешь выйти из режима join вызови /cancel'
		)
		return 1
	...
	if add_participant(meeting_id, user_id) == -2:
		update.message.reply_text(
			'Ты уже являешься участником этого мероприятия!'
		)
		return ConversationHandler.END

	update.message.reply_text(
		'Тебя добавили в участники! '
		'Теперь ты сможешь поучаствовать в принятии решений (/answer_questions)'
		' и точно не пропустишь всю важную информацию (/get_meeting_info).'
	)
	return ConversationHandler.END
